...
func GetTokenValue(tokenAddr string, conn *bind.ContractBackend) (float64, error) {
	var value float64
	var err error
	if chainlinkPriceAddr, ok := consts.TokenChainlinkProxyMap[tokenAddr]; ok {
		value, err = GetKnownTokenValue(chainlinkPriceAddr, conn)
	} else {
...
func GetKnownTokenValue(chainlinkPriceAddr string, conn *bind.ContractBackend) (float64, error) {
	chainlinkPriceCaller, err := chainlink.NewChainlinkPrice(common.HexToAddress(chainlinkPriceAddr), *conn)
	if err != nil {
